...
block DamperValves // _99_01_benchmark
  "Output signals for controlling VAV reheat box damper and valve position"

  parameter Real deltaMovAvg=5*60; // shiyab added

  Modelica.Blocks.Sources.Constant One_s(final k=1) // shiyab added
    "Constant one"; // shiyab added

  parameter Modelica.SIunits.TemperatureDifference DATMax; // shiyab added 190423
  ...
  Buildings.Controls.OBC.CDL.Continuous.MovingMean movAvg_TSup(delta=deltaMovAvg); // shiyab added
  ...
  Buildings.Controls.OBC.CDL.Continuous.MovingMean movAvg_TDisSet(delta=deltaMovAvg); // shiyab added
...
/*  Buildings.Controls.OBC.CDL.Continuous.AddParameter addPar(
    ...
    annotation (Placement(transformation(extent={{-260,-90},{-240,-70}}))); // shiyab removed 190423 */
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant addPar(k=DATMax); // shiyab added 190423
  ...
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hys2(
    final uHigh=0.05,
    final uLow=0.01)
//    final uHigh=0.11, // shiyab added
//    final uLow=0.03) // shiyab added
//    final uHigh=0.07, // shiyab added
//    final uLow=0.03) // shiyab added
...
//    final uHigh=0.11, // shiyab added
//    final uLow=0.03) // shiyab added
//    final uHigh=0.06, // shiyab added
//    final uLow=0.02) // shiyab added
    "Check if heating control signal is greater than 0"
    annotation (Placement(transformation(extent={{-260,-240},{-240,-220}})));
  Buildings.Controls.OBC.CDL.Continuous.MovingMean movAvg_uHea(delta=deltaMovAvg); // shiyab added

  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hys4(
    final uLow=TDisMin - 0.1,
    final uHigh=TDisMin + 0.1)
//    uLow=TDisMin - 0.6, // shiyab added
//    uHigh=TDisMin + 0.6) // shiyab added
//    uLow=TDisMin - 1.1, // shiyab added
//    uHigh=TDisMin + 1.1) // shiyab added
  ...
  Buildings.Controls.OBC.CDL.Continuous.MovingMean movAvg_TDis(delta=deltaMovAvg); // shiyab added

  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hys6(
    final uLow=-0.1,
    final uHigh=0.1)
//    uLow=-1.1, // shiyab added (around -0.7 is the edge if TSup and TRoo are almost equal)
//    uHigh=1.1) // shiyab added (around 0.7 is the edge if TSup and TRoo are almost equal)
//    uLow=-0.5, // shiyab added
//    uHigh=0.5) // shiyab added
...
/*  Buildings.Controls.OBC.CDL.Logical.TrueDelay truDel4(delayTime=600) // shiyab removed 190324 */
  Buildings.Controls.OBC.CDL.Logical.TrueDelay truDel4(delayTime=2.5*60) // shiyab added 190324
    "Check if the true input holds for certain time"
  ...
  Buildings.Controls.OBC.CDL.Integers.Sources.Constant unOcc(k=Buildings.Controls.OBC.ASHRAE.G36_PR1.Types.OperationModes.unoccupied)
  ...
  Buildings.Controls.OBC.CDL.Continuous.Gain VDisSetNor(final k=1/V_flow_nominal)
    "Normalized setpoint for discharge volume flow rate"
    annotation (Placement(transformation(extent={{240,160},{260,180}})));
  Buildings.Controls.OBC.CDL.Continuous.Gain VDisNor(final k=1/V_flow_nominal)
...
/*
  Modelica.Blocks.Sources.Step step( // shiyab added
    height=2.706,
    offset=286,
    startTime=24*60*60*35+(9*60*60)+45*60);
*/
equation
  connect(uCoo, lin.u)
...
/*  connect(uCoo, hys2.u)
  ...
  connect(movAvg_uCoo.y,hys2.u); // shiyab added
...
/*  connect(uHea, hys3.u)
    annotation (Line(points={{-340,-160},{-280,-160},{-280,-230},{-262,-230}},
      color={0,0,127})); // shiyab removed */
  connect(uHea,movAvg_uHea.u); // shiyab added
  connect(movAvg_uHea.y,hys3.u); // shiyab added
...
/*  connect(THeaSet, addPar.u)
    annotation (Line(points={{-340,-80},{-262,-80}}, color={0,0,127})); // shiyab removed 190423 */
  connect(uHea, lin3.u)
...
/*  connect(TRoo, addPar1.u)
    annotation (Line(points={{-340,-270},{-262,-270}}, color={0,0,127})); // shiyab removed */
  connect(movAvg_TRoo.y,addPar1.u); // shiyab added
...
/*  connect(TDis, hys4.u)
    annotation (Line(points={{-340,110},{-242,110}},
      color={0,0,127})); // shiyab removed */
  connect(TDis,movAvg_TDis.u); // shiyab added
  connect(movAvg_TDis.y,hys4.u); // shiyab added
...
/*  connect(TSup, add2.u1)
    annotation (Line(points={{-340,-50},{-300,-50},{-300,160},{-176,160},
      {-176,186},{-162,186}}, color={0,0,127})); // shiyab removed */
  connect(TSup,movAvg_TSup.u); // shiyab added
  connect(movAvg_TSup.y,add2.u1); // shiyab added
/*  connect(TRoo, add2.u2)
    annotation (Line(points={{-340,-270},{-296,-270},{-296,156},{-172,156},
      {-172,174},{-162,174}}, color={0,0,127})); // shiyab removed */
...
/*  connect(conTDisSet.y, add1.u1)
    annotation (Line(points={{-99,-92},{-80,-92},{-80,-240},{-140,-240},{-140,
          -264},{-122,-264}},   color={0,0,127})); // shiyab removed */
  connect(conTDisSet.y,movAvg_TDisSet.u); // shiyab added
  connect(movAvg_TDisSet.y,add1.u1); // shiyab added
  ...
  connect(conZer2.y, damPosUno.u1) annotation (Line(points={{-59,-12},{-20,-12},
          {-20,-32},{250,-32},{250,58},{278,58}},
                                             color={0,0,127}));
//  connect(One_s.y, damPosUno.u1);  // shiyab added
...
//  connect(movAvg_TDisSet.y, swi6.u1); // shiyab added
...
/*  connect(swi3.y, watValPosUno.u3) annotation (Line(points={{125,-92},{200,-92},
          {200,-48},{278,-48}},       color={0,0,127})); // shiyab removed 190324 */
  connect(conVal.y, watValPosUno.u3); // shiyab added 190324
...
//  connect(step.y,conVal.u_s); // shiyab added
annotation (
