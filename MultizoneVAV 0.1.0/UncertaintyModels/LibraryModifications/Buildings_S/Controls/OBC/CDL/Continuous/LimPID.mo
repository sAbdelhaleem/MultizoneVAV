within MultizoneVAV.UncertaintyModels.LibraryModifications.Buildings_S.Controls.OBC.CDL.Continuous;
block LimPID // _99_01_benchmark
...
/*  parameter Real k(
    min=0,
    unit="1") = 1 "Gain of controller"; // shiyab removed */
/*  parameter Modelica.SIunits.Time Ti(min=Constants.small) = 0.5
    "Time constant of integrator block"
    annotation (Dialog(enable=
          controllerType == CDL.Types.SimpleController.PI or
          controllerType == CDL.Types.SimpleController.PID)); // shiyab removed */
  Real k(min=0,unit="1") = 1 "Gain of controller"; // shiyab added
  Modelica.SIunits.Time Ti(min=Modelica.Constants.small) = 0.5 "Time constant of integrator block"; // shiyab added
...
/*  parameter Real y_reset=xi_start
    ...
    annotation(Dialog(enable=reset == CDL.Types.Reset.Parameter, group="Integrator reset")); // shiyab removed */

  Real y_reset=xi_start // shiyab added
    "Value to which the controller output is reset if the boolean trigger has a rising edge, used if reset == CDL.Types.Reset.Parameter"; // shiyab added
...
/*  Buildings.Controls.OBC.CDL.Continuous.IntegratorWithReset I( // shiyab removed */
  MultizoneVAV.UncertaintyModels.LibraryModifications.Buildings_S.Controls.OBC.CDL.Continuous.IntegratorWithReset I( // shiyab added
...
protected
  ...
  Buildings.Controls.OBC.CDL.Continuous.Add addSat(
    ...
    annotation (Placement(transformation(origin={80,-50},extent={{-10,-10},{10,10}},
      rotation=270)));
/*  Buildings.Controls.OBC.CDL.Continuous.Gain gainPID( // shiyab removed */
  MultizoneVAV.UncertaintyModels.LibraryModifications.Buildings_S.Controls.OBC.CDL.Continuous.Gain gainPID( // shiyab added
    final k=k) "Multiplier for control gain"
    annotation (Placement(transformation(extent={{30,-10},{50,10}})));

/*  Buildings.Controls.OBC.CDL.Continuous.Gain gainTrack(k=1/(k*Ni)) if with_I // shiyab removed */
  MultizoneVAV.UncertaintyModels.LibraryModifications.Buildings_S.Controls.OBC.CDL.Continuous.Gain gainTrack(k=1/(k*Ni)) if with_I // shiyab added
